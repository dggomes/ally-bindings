...
- The separate M1/M2 software probe never references Ally Bindings Core/HidSharp, opens no ASUS HID interface, installs/configures no driver and hides no device. It captures only F17/F18, creates virtual output only for an explicit timed bridge, and stores no arbitrary keys, username, machine name, process list or device path. Its package build rejects dormant hardware-write, driver-install and hiding symbols.
