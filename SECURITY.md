...
- The Armoury tap is a separate explicit diagnostic preview. After consent and UAC, the same executable extracts an embedded x64 DLL, verifies its SHA-256 while holding it without write/delete sharing, and injects it only into exact allowlisted ASUS-signed processes under trusted Windows/Program Files roots. The DLL observes target-scoped `HidD_SetFeature`, `HidD_SetOutputReport`, `WriteFile` and only the two HID SET `DeviceIoControl` codes, retains bytes solely for 50–64-byte `5A D1` writes on `0B05:1B4C` handles, and exports bounded aggregate filter counters without rejected payloads, hashes, exact nonmatching lengths, PID/path/raw QPC. It preserves the original API result and `LastError` and must unload cleanly before the run is accepted. It never changes either M1/M2 write gate.
...
- The separate M1/M2 software probe never references Ally Bindings Core/HidSharp, opens no ASUS HID interface, installs/configures no driver and hides no device. It captures only F11/F12, creates virtual output only for an explicit timed bridge, and stores no arbitrary keys, username, machine name, process list or device path. Its package build rejects dormant hardware-write, driver-install and hiding symbols.
- The former one-shot hardware-write command, native `HidD_SetFeature` import, approved writer workflow and writer package have been removed.
