...
public static class ArmouryTapProtocol
{
    ...
    public const byte OverflowRecordApi = 0xFF;
        ...
        int retainedRecordCount)
    {
        ...
        var targetCalls = (ulong)values[5] + values[6] + values[7];
        var allCalls = apiCounts.Aggregate(0UL, (sum, value) => sum + value);
        var classifiedCalls = (ulong)values[1] + values[2] + values[3] + values[4] + targetCalls;
        var counterSaturated = raw[1] == 1;
        if (!counterSaturated && (allCalls != classifiedCalls || values[8] > values[6] ||
            values[9] > values[6] || values[10] > values[9] || values[11] > values[10] ||
            (ulong)retainedRecordCount + values[12] != values[11]))
