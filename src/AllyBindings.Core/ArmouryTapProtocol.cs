...
public static class ArmouryTapProtocol
{
    ...
    public const byte SummarySchemaVersion = 2;
        ...
        int retainedRecordCount)
    {
        ...
        if (raw[60..].ContainsAnyExcept((byte)0) || apiCounts.Concat(values).Any(value => value > MaximumDiagnosticCounter))
        ...
        var lengthClassifiedCalls = (ulong)values[6] + values[7] + values[8];
        var handleValidationFailures = (ulong)values[1] + values[2] + values[3] + values[4] + values[5];
        var counterSaturated = raw[1] == 1;
        if (!counterSaturated)
        {
            if (allCalls != lengthClassifiedCalls || values[9] > values[7] ||
                handleValidationFailures > values[10])
                throw new InvalidDataException("Tap diagnostic counters violated their monotonic contract.");
            var readableBoundedCalls = (ulong)values[7] - values[9];
            var targetReportIdCalls = (ulong)values[10] - handleValidationFailures;
            if (values[10] > readableBoundedCalls || values[11] > targetReportIdCalls ||
                values[12] > values[11] || (ulong)retainedRecordCount + values[13] != values[12])
                throw new InvalidDataException("Tap diagnostic counters violated their monotonic contract.");
        }
        return new(processName, apiCounts[0], apiCounts[1], apiCounts[2], apiCounts[3], apiCounts[4],
            ...
            values[9], values[10], values[11], values[12], values[13], counterSaturated);
    }

    public static ArmouryTapPreFilterDiagnostics DecodeDiagnosticSummaryBytes(
        string processName,
        long packedApiCallCounts,
        uint setOutputReportCallCount,
        int deviceIoControlSetFeatureCallCount,
        byte[] raw,
        int retainedRecordCount) =>
        DecodeDiagnosticSummary(processName, packedApiCallCounts, setOutputReportCallCount,
            deviceIoControlSetFeatureCallCount, raw, retainedRecordCount);
...
public sealed record ArmouryTapPreFilterDiagnostics(
    ...
    uint UnvalidatedWriteHandleCount,
