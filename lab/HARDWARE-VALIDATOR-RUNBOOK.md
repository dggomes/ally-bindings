...
The executable is standalone: it does not reference Ally Bindings Core or compile the generic HID adapter. It contains no controller-state readback, reset builder, arbitrary mapping builder, parameterised report writer, or retry loop.
...
Never use a validator built by a pull request, an untrusted branch, or a third party. A checksum bundled inside a substituted ZIP is not provenance.
...
All three values—the computed hash, `OUTER-SHA256.txt`, and workflow summary—must match. After extraction, `SHA256SUMS.txt` must list exactly the other five packaged files once each; the validator’s safety job verifies this automatically.

## What it can do

- `inspect`: verify exact ASUS RC73XA DMI and exactly one compatible `VID_0B05/PID_1B4C` HID feature-report `0x5A` interface. It opens and validates native handle attributes/caps but never calls `GetFeature` or reads controller settings.
...
6. Verify the approved commit, outer ZIP SHA-256, internal package checksums, and logical packet hash.
...
The utility creates append-only audit records and an atomic `one-shot-claimed.json` marker before entering the HID operation. The same machine refuses another write unless development deliberately clears the marker after reviewing the evidence.
...
```text
%LOCALAPPDATA%\AllyBindings\HardwareValidator\
```
