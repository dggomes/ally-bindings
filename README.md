...
> **Project status: preview.** This source tree remains based on `v0.3.0-preview.20`. Standard-button remapping and public ASUS M1/M2 writes remain locked. The experimental one-shot hardware writer has been retired. The current hardware gate is a separate software-only probe: Armoury performs a one-time vendor-supported F12/F11 assignment, then the probe measures those keys and can temporarily bridge them to a virtual Xbox controller. It installs no driver, hides no device and never opens an ASUS HID interface. See [Current capabilities](#current-capabilities) and the [software-first validation plan](docs/HARDWARE-SPIKE.md).
...
- relies on Armoury's own virtual keyboard for the vendor-supported F12/F11 rear-button assignment and generates no assignment input;
- captures and optionally suppresses only F11/F12 key transitions through a low-level keyboard hook;
...
- can create one temporary virtual Xbox 360 controller, mapping M1/F12 to A and M2/F11 to B, only during an explicit timed `bridge` command;
...
- `src/AllyBindings.M1M2Probe` — F11/F12 capture, XInput and installed-driver inspection, and explicit temporary ViGEm bridge.
