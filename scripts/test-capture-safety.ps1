...
if ($app -notmatch 'CustomWritesApproved\)\s*\|\|\s*\(allowUnverifiedRecoveryReset\s*&&\s*ArmouryProtocolValidation\.RecoveryWritesApproved\)') {
    throw 'Windows backend construction is not guarded by both validation gates.'
}
