...
Ally Bindings is a local controller-mapping selector for the case where multiple streamed Xbox titles share one Windows Remote Play process. It does not modify Armoury Crate records or infer the game inside the video stream. M1/M2 firmware writes are implemented behind a source-level validation gate that remains closed. Passive capture is retained as a diagnostic, not the default route to product proof; a separately packaged private validator now owns the single fixed physical write experiment.
...
- `AllyBindings.HardwareValidator` is a separate private console artifact, not a runtime mode of the WPF application. It links only the narrow HID adapter, requires elevation, exact RC73XA/model/interface gates, one unchanged compatible interface, an interactive exact confirmation and a pre-write audit. Its only write is the fixed clean-room M1=A/M2=B packet; it has no restore/general-mapping path and never changes the application's source-level gates.
