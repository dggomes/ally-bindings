...
- `AllyBindings.HardwareValidator` is a standalone console artifact, not a runtime mode of the WPF application. It references neither Ally Bindings Core nor the generic HID adapter. It requires elevation, exact RC73XA/PID_1B4C same-handle validation, one compatible interface, interactive exact confirmation, a durable one-shot claim and append-only audits. Its only mutation is one literal M1=A/M2=B SET_FEATURE call; it has no readback, reset, general-mapping or retry path and never changes the application's source-level gates. Runnable artifacts are produced only by a manual main-branch workflow behind the `hardware-lab-approval` environment.
